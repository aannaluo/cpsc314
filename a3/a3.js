...
function initMotions() {
    ...
    myboxMotion.addKeyFrame(new Keyframe('pose B',2.0, [-3, 0, 0, -90]));
    myboxMotion.addKeyFrame(new Keyframe('pose C',4.0, [-3, 6, 0, 0]));
    myboxMotion.addKeyFrame(new Keyframe('pose D',6.0, [0, 6, 0, 0]));
    myboxMotion.addKeyFrame(new Keyframe('pose A',8.0, [0, 0, 0, 0]));
    ...
    handMotion.addKeyFrame(new Keyframe('left finger curl', 3.0, [0, 3,    0, 0, 0, -180,-90]));
    handMotion.addKeyFrame(new Keyframe('straight',         4.0, [0, 3,    0, 0, 0, -360, 0]));
    handMotion.addKeyFrame(new Keyframe('both fingers curl',4.5, [0, 3,    0, -90, -90, -180,-90]));
...
function onResourcesLoaded(){
 ...
 //                             i.e., creates references to the geometry, and not full copies ]
    ...
    meshes["teapot3"] = models.teapot.mesh.clone();
    ...
    meshes["teapot2"].position.set(1, 0, 5);
    meshes["teapot2"].scale.set(0.5, 0.5, 0.5);
    scene.add(meshes["teapot2"]);

    meshes["teapot3"].position.set(-1, 0, 5);
    meshes["teapot3"].scale.set(0.5, 0.5, 0.5);
    scene.add(meshes["teapot3"]);
